...
char BOOT_UPGRADE_FWNAME[128] = "Himax_firmware.bin";
EXPORT_SYMBOL(BOOT_UPGRADE_FWNAME);
...
/*Linden code for JLINDEN-283 by chenxin at 20221125 start*/
#define HIMAX_PROC_GESTURE_CONTROL	"gesture_control"
/*Linden code for JLINDEN-283 by chenxin at 20221125 end*/
...
void himax_charger_notify_work(struct work_struct * work)
{
	struct input_dev *input_dev = private_ts->input_dev;
	if (NULL == work) {
		E("%s : parameterwork are null !\n",__func__);
		return;
	}
	...
	if (USB_DETECT_IN == USB_detect_flag) {
		...
		printk(KERN_ERR "enter himax_charger_notify_work true\n");
		...
		printk(KERN_ERR "enter himax_charger_notify_work false\n");
		mutex_unlock(&input_dev->mutex);
	}
}
...
int charger_notifier_callback(struct notifier_block * nb, unsigned long val, void * v)
{
	...
	if (!strcmp(psy->desc->name, "usb") || !strcmp(psy2->desc->name, "pc_port")) {
		if(psy && psy2 && val == POWER_SUPPLY_PROP_STATUS) {
			...
			if (ret2 < 0) {
				E("Couldn't get POWER_SUPPLY_PROP_ONLINE RC2 = %d \n", ret2);
				return -ret2;
			}

			if (prop.intval == USB_DETECT_IN || prop2.intval == USB_DETECT_IN) {
				USB_detect_flag = 1;
			} else {
				USB_detect_flag = 0;
			}
			/*Linden code for JLINDEN-1276 by chenxin at 20230113 start*/
			//I("%s: himax usb = %d,pc usb = %d,callback USB_detect_flag =%d\n",__func__, prop.intval, prop2.intval,USB_detect_flag);
			//printk(KERN_ERR "himax usb = %d,pc usb = %d,USB_detect_flag=%d\n",prop.intval, prop2.intval,USB_detect_flag);
...
/*Linden code for JLINDEN-283 by chenxin at 20221125 start*/
static struct proc_dir_entry *Himax_gesture_entry = NULL;
/*Linden code for JLINDEN-283 by chenxin at 20221125 end*/
		...
		size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		temp_buf = kcalloc(len, sizeof(char), GFP_KERNEL);
		if (temp_buf != NULL) {
			if(ts->SMWP_enable == 1 && ts->gesture_cust_en[0] == 1)
				count = snprintf(temp_buf, PAGE_SIZE, "%d\n",1);
			else
				count = snprintf(temp_buf, PAGE_SIZE, "%d\n",0);
			if (copy_to_user(buf, temp_buf, len))
		...
		size_t len, loff_t *pos)
{
	...
	g_core_fp.fp_set_SMWP_enable(ts->SMWP_enable, ts->suspended);
	HX_SMWP_EN = ts->SMWP_enable;
	I("%s: SMART_WAKEUP_enable = %d,gesture_cust_en = %d.\n", __func__, HX_SMWP_EN, ts->gesture_cust_en[0]);

	return len;
}

static const struct proc_ops_name himax_gesture_fops = {
	owner_line
	.proc_op(read) = himax_gesture_read,
	.proc_op(write) = himax_gesture_write,
};
...
int himax_common_proc_init(void)
{
	...
	/*Linden code for JLINDEN-283 by chenxin at 20221125 start*/
	Himax_gesture_entry = proc_create(HIMAX_PROC_GESTURE_CONTROL, 0666, NULL, &himax_gesture_fops);
	if (Himax_gesture_entry == NULL) {
		I("%s: **himax create /proc/gesture_control Failed!\n", __func__);
		remove_proc_entry(HIMAX_PROC_GESTURE_CONTROL, NULL);
		return -ENOMEM;
	} else {
		I("%s: **himax create /proc/gesture_control Success!\n", __func__);
	}
	/*Linden code for JLINDEN-283 by chenxin at 20221125 end*/
...
static void himax_excp_hw_reset(void)
{
	...
	g_core_fp.fp_set_panel_direction(private_ts ->direction);
	g_core_fp.fp_set_game_mode(private_ts ->game_mode);
	/*Linden code for JLINDEN-3952 by chenxin at 20230306 start*/
	g_core_fp.fp_set_support_pen(private_ts ->support_pen);
	/*Linden code for JLINDEN-3952 by chenxin at 20230306 end*/
	/* Linden code for JLINDEN-11049 by dingying at 20231024 start */
	g_core_fp.fp_set_prevent_notch(private_ts->cmd_param);
...
static void himax_stylus_report(struct himax_ts_data *ts)
{
	...
	if (valid) {/*stylus down*/
		...
		ppen_info->coords.tool_type = (char)BTN_TOOL_PEN ;
...
/*Linden code for JLINDEN-3584 by chenxin29 at 20230302 start*/
#ifdef CONFIG_HQ_SYSFS_SUPPORT
      snprintf(ic_data->str_fw_ver,8,"%02X",ic_data->vendor_touch_cfg_ver);
      I("fw_ver:%d,char fw_ver:%s\n",ic_data->vendor_touch_cfg_ver,ic_data->str_fw_ver);
      hq_register_hw_info(HWID_CTP,(char *)ic_data->str_fw_ver);
#endif
/*Linden code for JLINDEN-3584 by chenxin29 at 20230302 end*/
END:
	ic_boot_done = 1;
...
#if defined(RESUME_FW_REQUEST_FROM_BUFFER)
	ts->g_fw_buf = kzalloc(sizeof(uint8_t) * (255 * 1024), GFP_KERNEL);
	if (ts->g_fw_buf == NULL) {
...
/*Linden code for JLINDEN-503 by chenxin at 20221214 start*/
	...
	if (ts->charger_detection_enable) {
		...
		if (ts->charger_detection == NULL) {
			E("failed to allocted memory for usb_charger_detection\n");
	...
	if (ts->charger_detection) {
		...
		if (!ts->charger_detection->himax_charger_notify_wq) {
			E("failed create_singlethread_workqueue\n");
		...
		if (ret) {
			E("Unable to register_charger_noti :%d\n", ret);
		...
		if (psy && psy2) {
			ret = power_supply_get_property(psy,POWER_SUPPLY_PROP_ONLINE, &prop);
			if (ret < 0) {
				E("couldn't POWER_SUPPLY_PROP_ONLINE :%d\n", ret);
				goto err_register_charger_notify_failed;
			}

			ret2 = power_supply_get_property(psy2,POWER_SUPPLY_PROP_ONLINE, &prop2);
			if (ret2 < 0) {
				E("couldn't POWER_SUPPLY_PROP_ONLINE2 :%d\n", ret2);
				goto err_register_charger_notify_failed;
			}
			...
			/*Linden code for JLINDEN-1276 by chenxin at 20230113 start*/
			//I("%s: himax usb = %d,pc usb = %d,USB_detect_flag = %d\n", __func__, prop.intval, prop2.intval,USB_detect_flag);
			//printk(KERN_ERR "himax usb = %d,pc usb = %d,2USB_detect_flag = %d\n",prop.intval, prop2.intval,USB_detect_flag);
			/*Linden code for JLINDEN-1276 by chenxin at 20230113 end*/
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
	...
	/*Linden code for JLINDEN-7255 by dingying3 at 20230411 start*/
	g_core_fp.fp_set_support_pen(private_ts ->support_pen);
	/*Linden code for JLINDEN-7255 by dingying3 at 20230411 end*/
	/*Linden code for JLINDEN-3604 by dingying3 at 20230418 start*/
	g_core_fp.fp_set_panel_direction(private_ts ->direction);
	/*Linden code for JLINDEN-3604 by dingying3 at 20230418 end*/
	/*Linden code for JLINDEN-9868 by dingying3 at 20230515 start*/
	g_core_fp.fp_set_game_mode(private_ts ->game_mode);
	/*Linden code for JLINDEN-9868 by dingying3 at 20230515 end*/
	/* Linden code for JLINDEN-11049 by dingying at 20231020 start */
	g_core_fp.fp_set_prevent_notch(private_ts->cmd_param);
	/* Linden code for JLINDEN-11049 by dingying at 20231020 end */
	himax_report_all_leave_event(ts);
