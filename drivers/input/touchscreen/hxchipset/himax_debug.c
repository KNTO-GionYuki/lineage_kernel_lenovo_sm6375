			...
			const char __user *buff, size_t len, loff_t *data)
{
	...
	switch(buf[0])
	{
		...
		case '1':
			...
			break;
		case '2':
			g_core_fp.fp_set_panel_direction(PANEL_LANDSCAPE_180);
			break;
		case '3':
...
{
	uint16_t pen_id_sta = g_core_fp.fp_read_pen_id_mode();
	ssize_t ret = 0;
	int i = 0;
	...
	for(i= 0;i< 7;i++)
		{
			seq_printf(s,"%02X", private_ts->pen_id[i]);
		}
	seq_puts(s, "\n");
			...
			const char __user *buff, size_t len, loff_t *data)
{
	...
	g_core_fp.fp_set_pen_id_mode(cmd_tmp[0]);
	return len;
}
			...
			const char __user *buff, size_t len, loff_t *data)
{
	...
	private_ts ->game_mode = cmd_tmp[0];
...
{
	uint8_t support_pen = g_core_fp.fp_read_support_pen();
	ssize_t ret = 0;
	/* Linden code for JLINDEN-11055 by dingying at 20230704 start */
	seq_printf(s,"%d\n", support_pen);
	/* Linden code for JLINDEN-11055 by dingying at 20230704 end */
			...
			const char __user *buff, size_t len, loff_t *data)
{
	...
	I("%s cmd is %c\n",__func__,buf[0]);
	sscanf(buf, "%d", &cmd_tmp[0]);
	g_core_fp.fp_set_support_pen(cmd_tmp[0]);
	return len;
}
			...
			const char __user *buff, size_t len, loff_t *data)
{
	...
	ret = sscanf(tmp_buf, "%d,%d,%d,%d", &private_ts->cmd_param[0], &private_ts->cmd_param[1], &private_ts->cmd_param[2], &private_ts->cmd_param[3]);
	...
	ret = g_core_fp.fp_set_prevent_notch(private_ts->cmd_param);
	I("%s: ret = %d\n",__func__,ret);
			...
			const char __user *buff, size_t len, loff_t *data)
{
	...
	/*Linden code for JLINDEN-316 by chenxin at 20221129 start*/
	int str_len = 0;
	/*Linden code for JLINDEN-316 by chenxin at 20221129 end*/
	...
	if ( fw_type == 0 ) {
		if (fw) {
				if (private_ts->g_fw_buf !=NULL) {
					private_ts->g_fw_len = fw->size;
					memcpy(private_ts->g_fw_buf, fw->data, fw->size);
...
#endif
fail_8:	remove_proc_entry(HIMAX_PROC_PREVENT_NOTCH_FILE, NULL);
/*Linden code for JLINDEN-3952 by chenxin at 20230306 end*/
fail_7: remove_proc_entry(HIMAX_PROC_GAME_MODE_FILE, NULL);
