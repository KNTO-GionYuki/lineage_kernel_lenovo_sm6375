...
static void himax_mcu_set_panel_direction(uint8_t direction)
{
	...
	/*Linden code for JLINDEN-3604 by dingying3 at 20230418 start*/
	/*
	if (private_ts ->direction == direction){
		return;
	}
	*/
	/*Linden code for JLINDEN-3604 by dingying3 at 20230418 end*/
	...
	switch (direction)
		{
			...
			case PANEL_LANDSCAPE_180:
				himax_parse_assign_cmd(PANEL_LANDSCAPE_180_HDSK_PWD,tmp_data, 4);
				himax_parse_assign_cmd(PANEL_LANDSCAPE_180_HDSK_PWD,back_data, 4);
				private_ts ->direction = PANEL_LANDSCAPE_180;
				break;
	...
	do {
		...
		g_core_fp.fp_register_read(pfw_op->addr_panel_direction,tmp_data1, DATA_LEN_4);
	...
	switch (pwd)
		{
			...
			case PANEL_LANDSCAPE_90_HDSK_PWD:
				...
				break;
			case PANEL_LANDSCAPE_180_HDSK_PWD:
...
static void himax_mcu_set_game_mode(uint8_t mode)
{
	...
	switch (mode)
		{
			case 0:
				himax_parse_assign_cmd(NORMAL_MODE_ENABLE,tmp_data, 4);
				himax_parse_assign_cmd(NORMAL_MODE_ENABLE,back_data, 4);
				break;
			case 1:
				himax_parse_assign_cmd(PEN_MODE_ENABLE,tmp_data, 4);
				himax_parse_assign_cmd(PEN_MODE_ENABLE,back_data, 4);
				break;
			case 2:
				himax_parse_assign_cmd(GAME_MODE_ENABLE,tmp_data, 4);
				himax_parse_assign_cmd(GAME_MODE_ENABLE,back_data, 4);
				break;
			default :
				himax_parse_assign_cmd(NORMAL_MODE_ENABLE,tmp_data, 4);
				himax_parse_assign_cmd(NORMAL_MODE_ENABLE,back_data, 4);
		...
		I("%s mode is %d.\n",__func__,mode);
	do {
		g_core_fp.fp_register_write(pfw_op->addr_game_mode,tmp_data, DATA_LEN_4);
		usleep_range(100, 101);
		g_core_fp.fp_register_read(pfw_op->addr_game_mode, tmp_data1,DATA_LEN_4);
...
static uint8_t himax_mcu_read_game_mode(void)
{
	uint8_t tmp_data[DATA_LEN_4] = {0};
	uint32_t pwd = 0;
	I("%s , now palm mode %d.",__func__,private_ts ->game_mode);
...
static uint8_t himax_mcu_read_support_pen(void)
{
	uint8_t tmp_data[DATA_LEN_4] = {0};
	uint32_t pwd = 0;

	I("%s , now pen mode %d.",__func__,private_ts ->support_pen);
	g_core_fp.fp_register_read(pfw_op->addr_support_pen,tmp_data,DATA_LEN_4);
	pwd = (uint32_t)((tmp_data[3] << 24) |
		   (tmp_data[2] << 16) |
		   (tmp_data[1] << 8) |
		   (tmp_data[0]));
	...
	switch (pwd)
		{
			case SUPPORT_PEN_DISABLE:
				private_ts ->support_pen = 0;
				I("%s pen mode is off \n",__func__);
				break;
			case SUPPORT_PEN_ENABLE:
				I("%s pen mode is on\n",__func__);
				private_ts ->support_pen = 1;
			default :
				private_ts ->support_pen = 1;
				I("%s default value, pen mode is on\n",__func__);
				break;
		}
	return private_ts ->support_pen;
}

static void himax_mcu_set_support_pen(uint8_t mode)
{
	uint8_t tmp_data[DATA_LEN_4],tmp_data1[DATA_LEN_4];
	uint8_t back_data[DATA_LEN_4];
	uint8_t retry_cnt = 0;
	...
	switch (mode)
		{
			case 0:
				himax_parse_assign_cmd(SUPPORT_PEN_DISABLE,tmp_data, 4);
				himax_parse_assign_cmd(SUPPORT_PEN_DISABLE,back_data, 4);
				break;
			case 1:
				himax_parse_assign_cmd(SUPPORT_PEN_ENABLE,tmp_data, 4);
				himax_parse_assign_cmd(SUPPORT_PEN_ENABLE,back_data, 4);
				break;
			default :
				himax_parse_assign_cmd(SUPPORT_PEN_DISABLE,tmp_data, 4);
				himax_parse_assign_cmd(SUPPORT_PEN_DISABLE,back_data, 4);
		...
		I("%s mode is %d.\n",__func__,mode);
	do {
		g_core_fp.fp_register_write(pfw_op->addr_support_pen,tmp_data, DATA_LEN_4);
		usleep_range(100, 101);
		g_core_fp.fp_register_read(pfw_op->addr_support_pen, tmp_data1,DATA_LEN_4);
		I("%s: tmp_data[0]=%d, mode=%d, retry_cnt=%d\n", __func__, tmp_data1[0],mode,retry_cnt);
		retry_cnt++;
	} while ((tmp_data1[3] != back_data[3]
		|| tmp_data1[2] != back_data[2]
...
static void himax_mcu_set_pen_id_mode(uint8_t mode)
{
	uint8_t tmp_data[DATA_LEN_4],tmp_data1[DATA_LEN_4];
	uint8_t back_data[DATA_LEN_4];
	uint8_t retry_cnt = 0;
	switch (mode)
		{
			case 0:
				himax_parse_assign_cmd(PEN_ID_RD_DIS,tmp_data, 4);
				himax_parse_assign_cmd(PEN_ID_RD_DIS,back_data, 4);
				break;
			case 1:
				himax_parse_assign_cmd(PEN_ID_RD_EN,tmp_data, 4);
				himax_parse_assign_cmd(PEN_ID_RD_EN,back_data, 4);
				break;
			default :
				himax_parse_assign_cmd(PEN_ID_RD_DIS,tmp_data, 4);
				himax_parse_assign_cmd(PEN_ID_RD_DIS,back_data, 4);
	...
	do {
		g_core_fp.fp_register_write(pfw_op->addr_pen_id_en,tmp_data, DATA_LEN_4);
		usleep_range(100, 101);
		g_core_fp.fp_register_read(pfw_op->addr_pen_id_en, tmp_data1,DATA_LEN_4);
		I("%s: tmp_data[0]=%d, mode=%d, retry_cnt=%d\n", __func__, tmp_data1[0],mode,retry_cnt);
		retry_cnt++;
	} while ((tmp_data1[3] != back_data[3]
		|| tmp_data1[2] != back_data[2]
...
static uint16_t himax_mcu_read_pen_id_mode(void)
{
	uint8_t tmp_data[4] = {0};
	uint8_t tmp_data_id[8] = {0};
	int m = 0,i= 0,zero_cnt = 0;
	uint32_t pwd = 0;
	uint8_t ret = RTN_AP_PEN_ID_NOT_START;
	uint16_t check_sum = 0;
	I("%s , now reading pen id.",__func__);
	m = g_core_fp.fp_register_read(pfw_op->addr_pen_id_en,tmp_data,4);
	if( m < 0) {
		E("%s read from IC is error.\n",__func__);
		ret = (uint8_t)RTN_AP_PEN_ID_ERR;
		return ret;
	}
	pwd = (uint32_t)((tmp_data[3] << 24) |
		   (tmp_data[2] << 16) |
		   (tmp_data[1] << 8) |
		   (tmp_data[0]));
	if ( pwd == PEN_ID_RD_DOING || pwd == PEN_ID_RD_EN) {
		I("%s FW is reading.\n",__func__);
		ret = (uint8_t)RTN_AP_PEN_ID_ERR;
		return ret;
	}else if( pwd == PEN_ID_RD_OVER ) {
		I("%s FW had read finish.\n",__func__);
	}else {
		I("%s FW not start yet.\n",__func__);
		ret = RTN_AP_PEN_ID_NOT_START;
		return ret;
	}
	m = g_core_fp.fp_register_read(pfw_op->addr_pen_id_data,tmp_data_id,8);
	if( m < 0) {
	...
	if (check_sum % 0x100 != 0) {
		...
		for ( i=0; i < 7;i++)
		{
			private_ts->pen_id[i]= tmp_data_id[i];
			I("%s:pen id read ok",__func__);
		}
		ret = (uint8_t)RTN_AP_PEN_ID_OK;
	}
	return ret;
}
#endif

/* Linden code for JLINDEN-11049 by dingying at 20231024 start */
static int himax_mcu_set_prevent_notch(int32_t *data)
{
	uint8_t tmp_data[DATA_LEN_4] = { 0 };
	uint8_t back_data[DATA_LEN_4] = { 0 };
	uint8_t retry_cnt = 0;
	...
	do {
		g_core_fp.fp_register_write(pfw_op->addr_prevent_notch, buf,
					    DATA_LEN_4);

		g_core_fp.fp_register_read(pfw_op->addr_prevent_notch, tmp_data,
					   DATA_LEN_4);
...
/* Linden code for JLINDEN-11049 by dingying at 20231024 end */
...
//	uint8_t *cfg_buf;
	...
	if (private_ts->cfg_buf == NULL) {
		private_ts->cfg_buf= kcalloc(cfg_sz, sizeof(uint8_t), GFP_KERNEL);
		if (private_ts->cfg_buf == NULL) {
	...
	for (i = 1; i < pnum; i++) {
		...
		memcpy(&private_ts->cfg_buf[info[i].cfg_addr - dsram_base],
	...
	cfg_crc_sw = g_core_fp.fp_Calculate_CRC_with_AP(private_ts->cfg_buf, 0, cfg_sz);
	do {
		g_core_fp.fp_write_sram_0f(sram_min, private_ts->cfg_buf, cfg_sz);
...
/* CORE_FW */
	...
	/*Linden code for JLINDEN-3952 by chenxin at 20230306 start*/
	g_core_fp.fp_set_support_pen = himax_mcu_set_support_pen;
	/*Linden code for JLINDEN-3952 by chenxin at 20230306 end*/
#if defined(Lenovo_pen_id_fun)
	g_core_fp.fp_set_pen_id_mode = himax_mcu_set_pen_id_mode;
	g_core_fp.fp_read_pen_id_mode = himax_mcu_read_pen_id_mode;
#endif
	g_core_fp.fp_read_game_mode = himax_mcu_read_game_mode;
	/*Linden code for JLINDEN-3952 by chenxin at 20230306 start*/
	g_core_fp.fp_read_support_pen = himax_mcu_read_support_pen;
	/*Linden code for JLINDEN-3952 by chenxin at 20230306 end*/
	g_core_fp.fp_set_prevent_notch = himax_mcu_set_prevent_notch;
...
/* CORE_FW */
	...
	/*Linden code for JLINDEN-3952 by chenxin at 20230306 start*/
	himax_parse_assign_cmd(fw_addr_support_pen,
		pfw_op->addr_support_pen,
		sizeof(pfw_op->addr_support_pen));
	/*Linden code for JLINDEN-3952 by chenxin at 20230306 end*/
	himax_parse_assign_cmd(fw_addr_prevent_notch,
		pfw_op->addr_prevent_notch,
		sizeof(pfw_op->addr_prevent_notch));
#if defined(Lenovo_pen_id_fun)
	himax_parse_assign_cmd(fw_addr_pen_id_wt,
		pfw_op->addr_pen_id_en,
		sizeof(pfw_op->addr_pen_id_en));
	himax_parse_assign_cmd(fw_addr_pen_id_rd,
		pfw_op->addr_pen_id_data,
		sizeof(pfw_op->addr_pen_id_data));
#endif
#if defined(HX_ULTRA_LOW_POWER)
