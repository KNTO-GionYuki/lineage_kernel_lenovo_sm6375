...
int himax_dev_set(struct himax_ts_data *ts)
{
	...
	ts->stylus_dev->name = "lenovo_ts,pen";
...
int himax_gpio_power_config(struct himax_platform_data *pdata)
{
	...
	if (pdata->gpio_reset >= 0) {
		...
		pdata->gpio_reset = g_tp_rst_gpio;
		/*linden code for JLINDEN-396 by yangjinmin at 20221206 end*/
...
int himax_chip_common_probe(struct spi_device *spi)
{
	struct himax_ts_data *ts;
	int ret = 0;
	I("******enter himax_chip_common_probe %s\n", __func__);
	/*linden code for JLINDEN-222 by yangjinmin at 20221130 start*/
	if(select_panel == -1) {
		I("select_panel = %d\n", select_panel);
		return -EPROBE_DEFER;
	} else if(select_panel == 0) {
		I("select_panel = %d\n", select_panel);
...
static int __init himax_common_init(void)
{
	...
	/* Linden code for JLINDEN-10584 by dingying at 20230607 start */
	if (select_tp_fw_mode == TP_BOE_FW_HIMIX) {
		strcpy(BOOT_UPGRADE_FWNAME,"Himax_firmware.bin");
		g_fw_boot_upgrade_name = BOOT_UPGRADE_FWNAME;
		I("FWNAME = Himax_firmware.bin \n");
	} else if (select_tp_fw_mode == TP_BOE_FW_HIMIX_TG) {
		strcpy(BOOT_UPGRADE_FWNAME,"Himax_firmware_tg.bin");
		g_fw_boot_upgrade_name = BOOT_UPGRADE_FWNAME;
		I("FWNAME = Himax_firmware_tg.bin \n");
	} else if (select_tp_fw_mode == TP_BOE_FW_HIMIX_GX) {
		strcpy(BOOT_UPGRADE_FWNAME,"Himax_firmware_gx.bin");
		g_fw_boot_upgrade_name = BOOT_UPGRADE_FWNAME;
		I("FWNAME = Himax_firmware_gx.bin \n");
	} else {
		E("no fw match himax driver");
	}
	/* Linden code for JLINDEN-10584 by dingying at 20230607 end */
