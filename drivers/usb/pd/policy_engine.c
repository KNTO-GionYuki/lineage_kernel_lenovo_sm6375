...
#define PD_MAX_SRC_VOLTAGE	 10000000
#define PD_MAX_SRC_CURRENT	 2200000
...
static int pd_select_pdo(struct usbpd *pd, int pdo_pos, int uv, int ua)
{
	...
	if (type == PD_SRC_PDO_TYPE_FIXED) {
		...
		/*Linden code for JLINDEN-320 by zhoujj21 at 20221123 start*/
		if (pd->requested_voltage > PD_MAX_SRC_VOLTAGE) {
			pdo_pos = pdo_pos -1;
		...
		/*Linden code for JLINDEN-320 by zhoujj21 at 20221123 start*/
		if (uv > PD_MAX_SRC_VOLTAGE) {
			uv = PD_MAX_SRC_VOLTAGE;
		}
		if (ua >= PD_MAX_SRC_CURRENT) {
			ua = PD_MAX_SRC_CURRENT;
		}
		/*Linden code for JLINDEN-320 by zhoujj21 at 20221123 start*/
