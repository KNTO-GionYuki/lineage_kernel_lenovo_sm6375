...
#include "../core/core.h"
#endif
/* Linden code for JLINDEN-227 by huangyi at 20221124 start */
#define CONFIG_MMC_SDHCI_BH201
#ifdef CONFIG_MMC_SDHCI_BH201
#include "sdhci-bayhub.h"
#endif
/* Linden code for JLINDEN-227 by huangyi at 20221124 end */
...
/* Linden code for JLINDEN-2178 by huangyi at 20230209 start */
#define SDHCI_MSM_MIN_CLOCK	400000
/* Linden code for JLINDEN-2178 by huangyi at 20230209 end */
...
/* Linden code for JLINDEN-227 by huangyi at 20221124 start */
	...
	if (bht_target_host(host)) {
		pr_info("%s: BH201 chip patch is used\n", mmc_hostname(host->mmc));
		sdhci_bh201_parse(host->mmc);
		msm_host->ggc.bh201_used = 1;
		...
		msm_host->ggc.bh201_used = 0;
		// host->mmc->bh201_used = 0;
	}
	#endif
/* Linden code for JLINDEN-227 by huangyi at 20221124 start */
...
/* Linden code for JLINDEN-227 by huangyi at 20221124 start */
	#ifdef CONFIG_MMC_SDHCI_BH201
	if (msm_host->ggc.bh201_used) {
		if (gpio_is_valid(msm_host->ggc.det_gpio))
			devm_gpio_free(&pdev->dev, msm_host->ggc.det_gpio);
		if (gpio_is_valid(msm_host->ggc.pwr_gpio))
			devm_gpio_free(&pdev->dev, msm_host->ggc.pwr_gpio);
	}
	#endif
/* Linden code for JLINDEN-227 by huangyi at 20221124 start */
