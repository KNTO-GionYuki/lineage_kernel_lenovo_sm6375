...
if [ ${TARGET_PRODUCT} == "linden_prc" ]; then
	QCOM_LCM_PRC_FRAG=${CONFIGS_DIR}/lcm.config
fi

####microtrust add prc
if [ ${TARGET_PRODUCT} == "linden_prc" ]; then
	QCOM_PRC_FRAG=${CONFIGS_DIR}/afw_prc.config
fi
...
export ARCH CROSS_COMPILE REAL_CC HOSTCC HOSTLD HOSTAR KERN_SRC KERN_OUT \
	CONFIGS_DIR BASE_DEFCONFIG QCOM_GKI_FRAG QCOM_QGKI_FRAG QCOM_DEBUG_FRAG QCOM_LCM_PRC_FRAG
